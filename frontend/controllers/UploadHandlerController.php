...
class UploadHandlerController extends Controller {
    ...
    public function actionIndex($id, $options = null, $initialize = true, $error_messages = null) {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $this->id = $id;
        $this->response = array();
        $this->options = array(
            'script_url' => $this->get_full_url() . '/' . $this->basename($this->get_server_var('SCRIPT_NAME')),
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . "/items/$id/",
            'upload_url' => $this->get_full_url() . "/items/$id/",
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = \yii\helpers\Url::base(true) . "/upload-handler/?id=$this->id&file=" . rawurlencode($file->name);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $generic_file_name = StaticController::generateRandomString() . StaticController::getFileExtension($file->name);
            $file_path = $this->get_upload_path($generic_file_name);
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $entity = new Item(['generic_file_name' => $generic_file_name, 'original_file_name' => $file->name, 'user_id' => $this->id]);
                    $entity->save();
                    $file->name = $generic_file_name;
                }
            } else {
    ...
    public function delete($print_response = true) {
        ...
        foreach ($file_names as $file_name) {
            ...
            if ($success) {
                $entity = Item::findOne(['generic_file_name' => $file_name]);
                if ($entity)
                    $entity->delete();
                foreach ($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
